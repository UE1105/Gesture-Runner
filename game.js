...
function drawSeamlessBackgroundLayer(img, scrollX, alignBottomY, layerHeight) {
    ...
    for (let x = -offset; x < canvas.width + drawWidth; x += drawWidth) {
        ctx.drawImage(img, x, drawY, drawWidth, layerHeight);
    }
}
...
function drawHomeScreen() {
    ...
    const displayList = [
        { idx: prevIdx, posX: canvas.width / 2 - 120, size: 55, alpha: 0.5 },
        { idx: currIdx, posX: canvas.width / 2,       size: 90, alpha: 1.0 },
        { idx: nextIdx, posX: canvas.width / 2 + 120, size: 55, alpha: 0.5 }
    ];
    ...
    ctx.fillText("◄", canvas.width / 2 - 180, 170);
    ctx.fillText("►", canvas.width / 2 + 180, 170);
...
document.addEventListener("keydown", function(event) {
    ...
    if (gameState === "PLAYING") {
        if (event.key === "ArrowLeft") {
            player.x -= 15;
            player.facing = "left";
            player.facingCooldown = 15; // 松開左鍵約0.25秒後轉回右邊
        }
        if (event.key === "ArrowRight") {
            player.x += 15;
            player.facing = "right";
